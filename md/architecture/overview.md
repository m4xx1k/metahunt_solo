...
- `AnalyticsModule` — global product-event boundary, and the only place a PostHog SDK instance exists (`PostHogClient`). Identity is `subscriptions.person_id`: every subscription has one from birth, an account claim rewrites it to `users.id`, and the claim emits the alias that merges the pre-account person — PostHog cannot stitch persons retroactively. Two stores, one author each: product mutations atomically write `analytics_outbox`, which `AnalyticsOutboxDispatcher` drains into `product_events` (Postgres only), while `PostHogClient` writes the eight-verb PostHog set directly. Feature services call domain methods on `AnalyticsService`; deterministic insert IDs make retries idempotent, and every early return in an analytics path logs at warn. `is_staff` on the PostHog person comes from `users.roles`, the same source the operator roster reads. Account UUIDs are used only as opaque distinct IDs, never as event properties; raw errors, Telegram identifiers, CV content, and full filter payloads are excluded. Dispatcher and PostHog failures cannot fail subscription or delivery behavior.
