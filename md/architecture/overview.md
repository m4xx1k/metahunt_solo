...
- `AnalyticsModule` — global PostHog boundary. Feature services call domain methods on `AnalyticsService`; server events use opaque subscription IDs, bounded filter summaries, and deterministic delivery event IDs. Raw Telegram identifiers and full filter payloads are excluded.
...
- Public privacy disclosure exists, but retention terms, analytics consent posture, account deletion, and the wording's legal ownership still require an explicit owner review before scaled CV traffic.
