...
- `AppModule` — imports `ConfigModule` (global, with env validation), `DatabaseModule` (global), `StorageModule`, `RssModule`, `MonitoringModule`. Registers `AppController` + `HealthController`. CORS enabled globally with `origin: "*"` (set in `main.ts`) so the web app can read `/monitoring` cross-origin.
...
- `NEXT_PUBLIC_API_URL` (declared in `apps/web/.env.example`) points at the ETL base URL. Used by Server-Component fetches in `apps/web/lib/api/monitoring.ts` (and any future API client) — must be set on the Vercel project per environment before deploying anything that calls the backend.
