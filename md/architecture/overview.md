...
- Auth: Telegram login only (Clerk removed). Consumer login mints the app's own JWT (Bearer, localStorage) — see [`md/runbook/telegram-auth.md`](../runbook/telegram-auth.md). The operator `app/(investigation)` route group is gated **client-side** by `AdminGuard` (requires a Telegram session with the `admin` role; membership via `ADMIN_TELEGRAM_IDS` on the API). There is no middleware/edge gate: operator pages are SSR-rendered and the read APIs stay open (see gaps), while the destructive `admin/taxonomy` + `rss` **mutations** are `@AdminOnly()` at the Nest layer. Landing/feed stay public.
...
- Operator UI (`(investigation)` group) is gated **client-side** by `AdminGuard` (Telegram `admin` role) — see Deployment → `@metahunt/web` above. The destructive ETL admin mutations (`admin/taxonomy` verify/hide/rename/merge, `rss/extract-missing`) are now `@AdminOnly()` (Telegram JWT + `admin`). Still open at the Nest layer: the operator **read** endpoints (so SSR pages can fetch server-side, where a localStorage Bearer can't reach). Proper edge/SSR gating of reads needs the session in a cookie — deferred. See [`rss-schedule-followups.md#a--production-hardening`](../journal/migrations/rss-schedule-followups.md#a--production-hardening) (A1).
