...
- `StorageModule` / `StorageService` — S3-compatible client over `@aws-sdk/client-s3` (`forcePathStyle: true`, so MinIO / Cloudflare R2 / AWS S3 all work via `STORAGE_*` env vars). Methods: `upload`, `download`, `ping` (HeadBucket, used by `/healthz`), plus `uploadBackup` / `listBackups` / `removeBackup`, which target `STORAGE_BACKUP_BUCKET` when set and fall back to `STORAGE_BUCKET`.
- `BackupModule` — weekly `pg_dump -Fc` of the whole database to `db-backups/<YYYY-MM-DD>/metahunt.dump` in object storage, on a Temporal schedule (`db-backup`, Sundays 02:00 UTC, `SKIP` overlap). Retention keeps the last 12 archives. It is both the only backup and the point-in-time slice `apps/lab` restores. Failure alerts to `ALERT_TELEGRAM_CHAT_ID` and leaves the workflow red — the dump is never silently skipped.
...
- Product analytics: a browser-local journey UUID begins anonymous tracking and is aliased only after verified account/Telegram identity to the opaque canonical person UUID. The API, Telegram handler, and digest worker reuse that person identity. No account profile fields, email, or Telegram identifier is sent to PostHog; behaviour reporting is PostHog-first while the first-party ledger remains a parity and delivery-reliability safeguard.
