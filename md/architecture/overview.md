...
- `AccountModule` — JWT-protected `/me` reads and mutations for CVs, subscriptions, linked identities, and permanent account deletion. Subscription names and CV-match criteria are owner-scoped snapshots; account responses expose taxonomy slugs while persistence keeps node ids. The account UI editor is admin-only during beta. `DELETE /me` removes identities, owned and same-chat subscriptions, notification history, ownership links, and final-owner derived candidates in one transaction.
