...
- `DedupModule` — groups postings into positions (ADR-0016). Pure pair rules (`match-rules.ts`: `vetoes` — override, company, requisition number, seniority, role, same-board different content; `isSame` — `exact` / `repost` / `cross_source` inside ±45 days) and complete-veto clustering (`clusters.ts`, deterministic, oldest first). One writer, `writePartition`, is used by the 5-minute `dedup-sweep` (rebuilds a pending vacancy's affected set) and by the `dedup plan` / `dedup apply` CLI (read-only full rebuild → `target.json` / `current.json`; apply is one transaction, `current.json` is the rollback). Embeddings (description-only text) retrieve ANN candidates and are one text signal. Operator verdicts live in `dedup_overrides` (`POST /operator/unique-vacancies/:groupId/detach`) and act as a veto in every rebuild. `dedup_reason` stores `{ rule, matchedAgainstVacancyId, titleSim, containment, cosine, decidedAt }`.
...
- `AnalyticsModule` — global product-event boundary, and the only place a PostHog SDK instance exists (`PostHogClient`). Identity is `subscriptions.person_id`: every subscription has one from birth, an account claim rewrites it to `users.id`, and the claim emits the alias that merges the pre-account person — PostHog cannot stitch persons retroactively. One store, one author: `PostHogClient` writes the eight-verb PostHog set directly from the feature services, and is the only writer of an analytics fact anywhere — the Postgres ledger retired in phase 4 of analytics-one-identity. `AnalyticsService` covers only the acts PostHog cannot be told about until an identity is resolved: an outbound click arrives as a subscription or a journey, never as a person. Every early return in an analytics path logs at warn. `is_staff` on the PostHog person comes from `users.roles`, the same source the operator roster reads. Account UUIDs are used only as opaque distinct IDs, never as event properties; raw errors, Telegram identifiers, CV content, and full filter payloads are excluded. Dispatcher and PostHog failures cannot fail subscription or delivery behavior.
...
- `AccountModule` — JWT-protected `/me` reads and mutations for CVs, subscriptions, linked identities, and permanent account deletion. Any owned subscription's name and filter criteria are editable, feed and legacy CV alike — `PATCH /me/subscriptions/:id` rejects an edit that collides with another live subscription's filter (409, same identity check `create()` makes). Account responses expose taxonomy slugs while persistence keeps node ids, plus a `refNames` map so the editor can label a selected role/skill/domain the market catalogs omit (no current vacancies). `DELETE /me` removes identities, owned and same-chat subscriptions, notification history, ownership links, and final-owner derived candidates in one transaction.
...
- `schema/*` — tables: `sources`, `rss_ingests`, `rss_records` (bronze); `companies`, `company_identifiers`, `nodes` (role/skill/domain taxonomy; `slug` is the URL-facing id — `?roles=backend-engineer` — minted once at ingest, immutable on rename, unique per `(type, slug)`; slugs resolve back to ids at the feed/ranking/cv/subscription API boundary via `NodeSlugResolver`), `node_aliases`, `vacancies`, `vacancy_nodes` (silver); `node_tech_meta` (LLM-classified skill metadata category/stack/is_core/generic — gates reverse-ATS recs + ranking demote, ADR-0010); `users`; `digest_deliveries` (retry-stable delivery envelopes and progress). `subscriptions` stores a mutable display name, an independent criteria snapshot, delivery state, nullable Telegram profile fields, `person_id` (the analytics identity), and `journey_id` — an unconstrained browser journey id since `analytics_journeys` was dropped, which is what ties an anonymous visit's `?j=` tap to a subscriber in PostHog. **Position read model** (ADR-0015) — three curated regular views that are the default entry point for every public product/analytics read: `postings` (one row per source observation over `vacancies`; explicit columns, identity renamed `posting_id`, `position_id` kept visible), `positions` (exactly one row per `unique_vacancies.id` — the **canonical** posting's facts plus `representative_posting_id` as a display/link pointer, `first_observed_at`/`last_source_activity_at`/`posting_count`/`source_count` from the group), and `position_nodes` (`position_id → node_id`, `is_required`; the canonical posting's taxonomy links, required and optional alike). The views hold every row: eligibility (`ELIGIBLE_POSITION`), VERIFIED gates, and time windows stay explicit consumer rules. Posting-grain reads survive only for ingestion/source health, extraction and dedup diagnostics, moderation/audit, apply/source URLs, and calibration-sensitive scoring pending MET-139 — each marked `POSTING-GRAIN-EXEMPT`. Enforcement of that marker is moving from a jest source-scan (`posting-grain-guard.spec.ts`, removed) to an ESLint rule — [tracker](../journal/migrations/test-slop-cleanup.md). Materialized views: `node_stats` (IDF weights), `node_skill_cooc` (one unordered pair of VERIFIED, REQUIRED skills per canonical position; support, conditional probabilities, lift, and NPMI — reads through `position_nodes`). Regular view: `track_counts` (per-track eligible **Position** count). Enums: `seniority`, `work_format`, `employment_type`, `english_level`, `currency`, `engagement_type`, `node_type`, `node_status`, `skill_category`.
- `market_snapshots`, `market_snapshot_positions`, `market_snapshot_position_nodes` — append-only Position-grain observations captured transactionally after each ingest refresh. A snapshot stores the full live `positions` row as JSON plus its canonical node links; database triggers reject all updates and deletes, so later source, canonical, or taxonomy changes cannot rewrite history.
- `migrations/*` + `migrations/meta/*` — drizzle SQL + snapshots/journal.
- Backups — Railway scheduled volume backups on the Postgres service (daily + weekly). They restore only into the same project and environment, so they cover data loss, not off-Railway copies; the lab's point-in-time slices come from on-demand `pg_dump` (see `apps/lab/README.md`).
...
- Auth: Google Identity Services and Telegram bot deep links mint the app's own JWT (Bearer, localStorage). Providers can be linked from `/me`; conflicts return 409 and never silently merge accounts. See [`md/runbook/auth.md`](../runbook/auth.md). The operator console is client-gated by `AdminGuard`, with the same JWT + role enforced server-side. Login/logout also sync an httpOnly session cookie for server-rendered console reads. Landing/feed stay public.
...
- Product analytics: `$pageview` and autocapture cover pages and clicks with no product code; a custom event exists only for a fact an outsider watching URLs and clicks could not infer. The browser stays anonymous until `identify()` merges its history onto `users.id`; the browser journey UUID reaches the subscription at create time and is aliased to its person. The API, Telegram handler, and digest worker all capture on the same person id. No account profile fields, email, or Telegram identifier is sent to PostHog; the first-party ledger remains a parity and delivery-reliability safeguard until it reproduces PostHog's counts for seven consecutive days.
...
- Extraction uses the direct DeepSeek API when `EXTRACTOR_PROVIDER=baml`; OpenAI embeddings remain in the dedup candidate-retrieval path. Empirical extraction and match quality still need real-user sampling.
