...
- `ProductAnalyticsModule` — administrator-only first-party funnel and identity-health API at `/admin/product-analytics/overview`. It aggregates pseudonymous journeys, critical product events, subscription state, legacy coverage, account-to-journey joins, delivery evidence, and correlation gaps without selecting Telegram identifiers or profile data. The client-only operator page at `/product-analytics` fetches this API with the existing Bearer session.
...
- Operator UI (`(investigation)` group) is client-gated by `AdminGuard` (Telegram `admin` role), and operator API controllers enforce `@AdminOnly()` server-side. Proper edge/SSR gating of the operator UI still needs the session in a cookie; it is a web delivery concern rather than an API authorization bypass.
- The production event chain has not yet been verified as one reproducible landing → linked subscription → first digest → digest click funnel, and no 7/30/90-day acquisition baseline is recorded in the repository.
