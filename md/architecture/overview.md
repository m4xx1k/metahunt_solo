...
- `AnalyticsModule` — global PostHog boundary. Feature services call domain methods on `AnalyticsService`; server events use opaque subscription IDs, bounded filter summaries, and deterministic delivery/evaluation IDs. `digest_evaluated` exposes empty versus matching runs and first-digest state; `digest_delivery_failed` exposes only a bounded failure kind. Raw errors, Telegram identifiers, account UUIDs, and full filter payloads are excluded; login emits an anonymous event instead of identifying the account.
