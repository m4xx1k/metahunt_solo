...
- `AppModule` — imports `ConfigModule` (global, with env validation), `DatabaseModule` (global), `StorageModule`, `TemporalInfraModule`, `RssModule`, `LoaderModule`, `VacanciesModule`, `TaxonomyModule`, `MonitoringModule`. Registers `AppController` + `HealthController`. CORS enabled globally with `origin: "*"` (set in `main.ts`) so the web app can read `/monitoring` and `/vacancies` cross-origin.
...
- `schema/*` — tables: `sources`, `rss_ingests`, `rss_records` (bronze); `companies`, `company_identifiers`, `nodes`, `node_aliases`, `vacancies`, `vacancy_nodes` (silver). Enums: `seniority`, `work_format`, `employment_type`, `english_level`, `currency`, `engagement_type`, `node_type`, `node_status`.
...
- BAML extraction prompt emits long-tail SKILL noise (variant spellings, generic categories, fluff). Plan: [`todo/baml-extraction-prompt-tuning.md`](../../todo/baml-extraction-prompt-tuning.md). Stage 06.
