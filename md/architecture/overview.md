...
- `AppModule` — imports `ConfigModule` (global, with env validation), `DatabaseModule` (global), `StorageModule`, `RssModule`, `LoaderModule`, `MonitoringModule`. Registers `AppController` + `HealthController`. CORS enabled globally with `origin: "*"` (set in `main.ts`) so the web app can read `/monitoring` cross-origin.
