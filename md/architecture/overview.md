...
- `AppModule` — imports `ConfigModule` (global, with env validation), `DatabaseModule` (global), `StorageModule`, `TemporalInfraModule`, `RssModule`, `LoaderModule`, `VacanciesModule`, `TaxonomyModule`, `MonitoringModule`, `ExtractionCostModule`. Registers `AppController` + `HealthController`. CORS enabled globally with `origin: "*"` (set in `main.ts`) so the web app can read `/monitoring`, `/vacancies` and `/extraction-cost` cross-origin.
