...
- `dedup-cli embed --force` — **done 2026-07-27**: `processed=13488 embedded=13488 skipped=0` in 11m32s. Took three attempts: the first two drained the OpenAI quota because `--force` was an infinite loop (`WHERE true ... LIMIT 100`, no cursor — every turn re-fetched the same page and paid for it; one run reached 68100 "embedded" on a 13488-row table). Fixed with a keyset cursor on `v.id` plus three tests that the old code cannot pass, since it never terminates. Cost of finding it out the hard way: roughly 90k embeddings instead of 13.5k.
- `buildEmbeddingText` puts `Role:` and `Skills:` inside the hashed text, so renames invalidate `embedding_source_hash`. The non-force fetch predicate never selects an already-embedded row, so the hash would stay wrong forever. The hash filter still gates the OpenAI call, so only genuinely changed rows cost money. Note changed embeddings shift ANN neighbours and can move `unique_vacancy_id` groups — re-running `dedup-cli resolve` is a decision, not a default.
...
the first real production run. That is evidence of a run, not a durable applied-plan record; a
second apply against an unknown partial state is not safe.

The owner-approved, read-only preflight must preserve the plan SHA and CLI JSONL audit, then have
exactly one of these outcomes:

| outcome          | evidence                                                                         | next action                                          |
| ---------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------- |
| Unapplied        | dry-run resolves every operation as `APPLY`, with only intentional hide warnings | schedule the existing migration window               |
