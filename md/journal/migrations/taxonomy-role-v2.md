...
- `dedup-cli embed --force` — **started 2026-07-27, died at 12100 of 13461 on an OpenAI `429 insufficient_quota`.** Top up the account and re-run. Note a plain (non-force) re-run picks up **nothing**: the fetch predicate is `embedding IS NULL OR embedding_model IS DISTINCT FROM <model>` and neither holds, so `--force` is the only way to reach the remainder — it redoes all 13461 (~$0.09, `skipped 0` in the log, the hash filter does not gate the call the way the earlier analysis assumed). Nothing is broken meanwhile: `dedup-cli resolve` was deliberately not run, so all 11385 groups are unchanged, and ~1400 rows simply keep the stale-hash condition that every row had before this work started.
