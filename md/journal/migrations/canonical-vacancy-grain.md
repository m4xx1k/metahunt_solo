...
### Phase 1a — expand (schema only, no behaviour change)

Migration adds, all nullable / defaulted so the running code keeps working:

```sql
-- NULL = not resolved yet; a timestamp = resolved, and when. No enum: a time
-- answers strictly more than a status at the same cost, and cannot rot.
ALTER TABLE vacancies
  ADD COLUMN deduplicated_at timestamptz;

-- rename the two existing columns to say what they actually hold (source's
-- claim, not when we saw anything), and add the trustworthy first-appearance fact
ALTER TABLE unique_vacancies
  RENAME COLUMN first_seen_at TO first_published_at;
ALTER TABLE unique_vacancies
  RENAME COLUMN last_seen_at  TO last_published_at;

ALTER TABLE unique_vacancies
  ADD COLUMN representative_vacancy_id uuid REFERENCES vacancies(id),
  ADD COLUMN first_loaded_at timestamptz;   -- MIN(loaded_at): the only bump-proof time axis

-- everything already grouped counts as resolved; the transient nulls stay pending
UPDATE vacancies SET deduplicated_at = updated_at WHERE unique_vacancy_id IS NOT NULL;

CREATE INDEX vacancies_pending_dedup_idx ON vacancies (id) WHERE deduplicated_at IS NULL;
CREATE INDEX unique_vacancies_last_published_idx ON unique_vacancies (last_published_at DESC);
```

Backfill of the three rollup columns runs as a one-shot statement in the same migration (8 876 groups —
one pass, no batching needed at this size).

**DoD 1a**
- [ ] `pnpm db:migrate` applies cleanly on a **restored copy** of the prod dump, not only on the dev DB.
- [ ] Journal entry present for the new tag (`grep` the tag in `meta/_journal.json`) — the 0039 trap.
- [ ] Post-migration invariant holds: `count(*) FROM unique_vacancies WHERE representative_vacancy_id IS NULL` = 0.
- [ ] `last_published_at` of every group equals `max(coalesce(published_at, loaded_at))` of its members — asserted by query, not by eye.
- [ ] Existing suite green with **no source changes**: `pnpm test:etl:int`. Phase 1a must be invisible.
