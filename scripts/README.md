...
3. **A plan-driven driver** if the same *shape* of operation will recur — the driver is a tool and stays, the plan is data and gets archived. `apps/etl/src/admin/taxonomy/taxonomy-migrate.cli.ts` is the example: the CLI is permanent, each `plans/*.plan.json` is a one-shot that moves to `plans/_done/` once applied.
