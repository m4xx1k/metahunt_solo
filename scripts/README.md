...
| `test-slop-check.mjs`            | test-quality guard — flags regex-over-source specs, assertion-free tests, faked Drizzle chains, `toHaveBeenCalled`-only tests. `lint-staged` pre-commit + a Claude Code hook. `md/journal/migrations/test-slop-cleanup.md` |
...
| `dev-jwt.sh`                     | mints a session JWT for an existing local user (`pnpm dev:jwt`) so an agent can hit any `@metahunt/etl` route directly — no browser login round-trip                           |
...
| `posthog-founder-setup.ts`  | verified/applied the MET-114 founder dashboard (naming, timezone, transition Action, 9 saved insights) in PostHog. Removed with the runbook it automated — that dashboard is superseded, not the current analytics home                            | `md/journal/migrations/met-114-analytics-reset.md`                                    | 2026-09-01 |
