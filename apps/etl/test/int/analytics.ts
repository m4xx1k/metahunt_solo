...
import { SubscriberIdentityStore } from "../../src/platform/analytics/subscriber-identity.store";
...
// No-op analytics for tests that construct a service needing one but don't
// assert on emitted events.
export function noopAnalytics(db: DrizzleDB): AnalyticsService {
  return new AnalyticsService(new SubscriberIdentityStore(db), dormantPostHog());
}
