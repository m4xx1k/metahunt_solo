...
WITH params AS (
  SELECT 'rep'::text AS rule, 25::int AS min_skill_support, 10::int AS min_pair_support,
         150::int AS min_role_positions,
         0.5::float8 AS substitute_min, 0.2::float8 AS complement_max
),
...
edge_rows AS (
  SELECT jsonb_agg(jsonb_build_object(
           ...
           'substitutePairs', e.substitute_positions,
           'substituteRate',  round(e.substitute_rate::numeric, 4),
           'complementNpmi',  round(e.complement_npmi::numeric, 4),
           -- Thresholds calibrated against pair-relations.json on the anyOf
           -- cohort: no hand-labelled SUBSTITUTE lands at <=0.2, no COMPLEMENT at >=0.5.
           'observed', CASE
             WHEN e.substitute_rate >= p.substitute_min THEN 'SUBSTITUTE'
             WHEN e.substitute_rate <= p.complement_max THEN 'COMPLEMENT'
             ELSE 'MIXED'
           END
...
SELECT jsonb_build_object(
  'contract', jsonb_build_object(
    ...
    'cohort',           'positions extracted under the anyOf contract (metalab_cohort)',
    'substituteMin',    (SELECT substitute_min FROM params),
    'complementMax',    (SELECT complement_max FROM params),
    'livenessClaim',    'none',
  ...
  'provenance', jsonb_build_object(
    'snapshot',      'Railway prod pg_dump, 2026-09-25',
    'corpusStart',   (SELECT min(v.published_at)::date FROM metalab_cohort c
                        JOIN unique_vacancies uv ON uv.id = c.position_id
                        JOIN vacancies v ON v.id = uv.canonical_vacancy_id),
    'corpusEnd',     (SELECT max(v.published_at)::date FROM metalab_cohort c
                        JOIN unique_vacancies uv ON uv.id = c.position_id
                        JOIN vacancies v ON v.id = uv.canonical_vacancy_id),
    'cohortPositions', (SELECT count(*) FROM metalab_cohort),
  ...
  -- Every VERIFIED skill name, below the support floor too: lets the curated
  -- labels tell a renamed skill (drift) from one that is merely rare here.
  'vocabulary',  (SELECT jsonb_agg(canonical_name ORDER BY canonical_name)
                  FROM nodes WHERE type = 'SKILL' AND status = 'VERIFIED')
);
