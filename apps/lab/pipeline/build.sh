...
#
#   pnpm --filter @metahunt/lab lab:data          # → src/data/{core,edges,roles}.json
#
# This is the only thing in the lab that touches a database, and it runs when a
# human asks it to. The app itself fetches the emitted JSON, so the UI can never
...
APP="$(dirname "$HERE")"
# Fetched at runtime, so served as static assets, not imported (guardrail 5).
OUT_DIR="${1:-$APP/public/data}"
CURATED="$APP/src/data/pair-relations.json"
PSQL="$HERE/psql.sh"
...
step "04 — emit raw graph document"
tmp="$(mktemp)"
...
node -e 'const fs = require("node:fs");
  ...
  if (!g.nodes?.length || !g.edges?.length) throw new Error("artifact is empty");' "$tmp"

step "05 — split, fold curated labels, write core/edges/roles"
# assemble.mjs resolves pair-relations.json against these nodes and exits 1 on
# an unresolved label or an over-budget core.json, leaving the old files intact.
mkdir -p "$OUT_DIR"
node "$HERE/assemble.mjs" "$tmp" "$OUT_DIR" "$CURATED"
rm -f "$tmp"
step "done → $OUT_DIR/{core,edges,roles}.json"
