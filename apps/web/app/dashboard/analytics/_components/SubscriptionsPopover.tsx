...
// Compact count as the trigger; per-subscription detail (track, feed/cv,
// created date) lives in the popover instead of eating a fat table column.
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          className="inline-flex items-center gap-1.5 border border-border bg-bg-elev px-2 py-1 font-mono text-2xs uppercase tracking-[0.12em] text-text-secondary transition-colors hover:border-accent hover:text-accent"
        >
      ...
      <PopoverContent aria-label="subscription detail">
          ...
          {subscriptions.map((sub) => (
            ...
            >
              ...
              <span className="text-2xs uppercase tracking-[0.12em] text-text-muted">
                {sub.isActive ? "active" : "deactivated"} · {formatDateOnly(sub.createdAt)}
              </span>
            </li>
          ))}
        </ul>
