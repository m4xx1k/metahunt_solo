    ...
    <Card className={isCrossSource ? "!border-accent" : undefined}>
      <header className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 font-mono text-xs text-text-muted">
          ...
          (group.salaryRange.min !== null || group.salaryRange.max !== null) ? (
            <span>salary: {formatSalaryRange(group.salaryRange)}</span>
          ) : null}
          <span>published: {formatDateRange(group.firstSeenAt, group.lastSeenAt)}</span>
        </div>
      </header>
      ...
      <details className="group/details">
        <summary className="flex cursor-pointer list-none items-center justify-between border-t border-border pt-4 font-mono text-2xs uppercase tracking-wider text-text-muted hover:text-text-primary">
          <span>
            {group.vacancyCount === 1
              ...
              : `${group.vacancyCount} postings + why they merged →`}
          </span>
        ...
        <div className="mt-4 flex flex-col gap-4">
          {group.members.map((m) => (
            <div key={m.vacancyId} className="flex flex-col gap-3 border-t border-border pt-4">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <div className="flex flex-wrap items-baseline gap-2">
                  <Badge variant={m.isCanonical ? "accent" : "dark"}>{m.source.displayName}</Badge>
                  {m.isCanonical ? (
