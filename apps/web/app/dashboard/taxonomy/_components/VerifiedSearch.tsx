...
import { taxonomyApi, type FuzzyMatch, type NodeType } from "@/lib/api/taxonomy";
...
export function VerifiedSearch({ sourceId, type }: { sourceId: string; type: NodeType }) {
  ...
  const handleChange = useCallback((e: ChangeEvent<HTMLInputElement>) => setQ(e.target.value), []);
  ...
  return (
    <div className="flex flex-col gap-2">
      <input
        ...
        placeholder="name or alias (min 2 chars)…"
        aria-label="search a verified entry to merge into"
      ...
      {active && state.kind === "ok" ? (
        <FuzzyMatchList sourceId={sourceId} matches={state.matches} emptyLabel="nothing found" />
      ) : null}
    </div>
  );
}
