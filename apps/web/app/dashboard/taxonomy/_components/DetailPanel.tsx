...
export async function DetailPanel({ nodeId }: { nodeId: string }) {
  ...
  try {
    [node, fuzzy] = await Promise.all([taxonomyApi.node(nodeId), taxonomyApi.fuzzyMatches(nodeId)]);
  } catch (e: unknown) {
    ...
    <PanelShell>
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2 font-mono text-xs">
          ...
          <span className="text-text-muted">created · {formatDateTime(node.createdAt)}</span>
        </div>
      </div>
      ...
      <Section label={`used in · ${node.vacancyCount} vacancies`}>
        {node.sampleVacancies.length === 0 ? (
          <p className="font-mono text-xs text-text-muted">no examples</p>
        ) : (
          <ul className="flex flex-col gap-1 font-mono text-xs">
            {node.sampleVacancies.map((v) => (
              <li key={v.id} className="flex items-baseline gap-2 truncate" title={v.title}>
      ...
      <Section label="similar verified entries">
        <FuzzyMatchList
          sourceId={node.id}
      ...
      <Section label="search verified to merge into">
  ...
  return (
    <PanelShell>
      <p className="font-mono text-sm text-text-muted">
        pick an entry on the left for detail and moderation actions
      </p>
    </PanelShell>
  );
}
