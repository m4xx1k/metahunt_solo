...
export function AxisBar({ axis, coverage }: { axis: AxisKey; coverage: AxisCoverage }) {
  const total = coverage.total;
  const missingPct = total > 0 ? (coverage.missing / total) * 100 : 0;
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <StackedBar
        ...
        total={total}
        height={20}
        ariaLabel={`coverage for ${AXIS_LABEL[axis]}`}
      />
    </div>
  );
}
