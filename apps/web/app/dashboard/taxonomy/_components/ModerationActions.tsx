...
import { TaxonomyApiError, taxonomyApi, type NodeDetail } from "@/lib/api/taxonomy";
...
export function ModerationActions({ node }: { node: NodeDetail }) {
  ...
  const handleVerify = useCallback(() => run(() => taxonomyApi.verify(node.id)), [node.id, run]);

  const handleHide = useCallback(() => run(() => taxonomyApi.hide(node.id)), [node.id, run]);
      ...
      try {
        ...
        if (e instanceof TaxonomyApiError && e.status === 409) {
          ...
          if (targetId) {
            setAction({
              busy: false,
              error: `"${newName}" already exists — merge into it instead`,
            });
...
function extractMergeTargetId(body: unknown): string | null {
  ...
  const direct = (body as { suggestion?: { mergeTargetId?: unknown } }).suggestion?.mergeTargetId;
  if (typeof direct === "string") return direct;
  const inner = (body as { message?: unknown }).message;
  if (typeof inner === "object" && inner !== null) {
    const nested = (inner as { suggestion?: { mergeTargetId?: unknown } }).suggestion
      ?.mergeTargetId;
